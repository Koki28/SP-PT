...
public class SendingData {
	 ...
	 */
	public static void sendData(int time, int data, List <Node> dijkstra, Node stackedNode, Node prime) {
		
		int loss, sent = 0; int second = time; int packageData = data;
	 
		Node last = dijkstra.get(dijkstra.size() - 1); Node one = dijkstra.get(0); Node two = dijkstra.get(1); Node first;
		
		if (prime == null) { first = dijkstra.get(0); } else { first = prime; }
			
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("simulation.txt", true))) {
				
			System.out.println("\n--- second " + second + " ---"); System.out.println("Sending data about size <" + packageData + "> from node " +  one.getId() + " to node " + two.getId() + ": ");
			...
			System.out.println("transmittance " + one.getId() + " -> " + two.getId() + ": " + transmittance); System.out.println("faulting " + one.getId() + " -> " + two.getId() + ": " + faulting);
			...
			if(transmittance > packageData) {
				...
				if(((double) packageData / (double) transmittance) > faulting && rand < 0.5) {
					...
					System.out.println("- Overload! -  Half ammount of data must be sent again (" + loss + ")."); bw.write("- Overload! -  Half ammount of data must be sent again (" + loss + ").\n\n");
				...
				System.out.println("From node " + one.getId() + " to node " + two.getId() + " was sent " + packageData + " amount of data."); bw.write("From node " + one.getId() + " to node " + two.getId() + " was sent " + packageData + " ammount of data.\n");
				...
				int remainingData = packageData - transmittance; sent = transmittance;
				...
				if (faulting != 1.0 && rand < 0.5) {
					...
					System.out.println("\n--- second " + second + " ---"); bw.write("--- second " + second + " ---\n");
				...
				System.out.println("From node " + one.getId() + " to node " + two.getId() + " was sent " + sent + " ammount of data."); bw.write("From node " + one.getId() + " to node " + two.getId() + " was sent " + sent + " ammount of data.\n");
				...
				if (!one.stack.addMemory(remainingData)) {
					...
					System.out.println("Memory of node " + one.getId() + " overflowed! Data must be sent again (" +  remainingData + ")."); bw.write("Memory of node " + one.getId() + " overflowed! Data must be sent again (" +  remainingData + ").\n");
					...
					System.out.println(remainingData + " ammount of data was saved to memory stack of node " + one.getId() + "."); bw.write(remainingData + " ammount of data was saved to memory stack of node " + one.getId() + ".\n");
	 ...
	 */
	public static void sendData(int time, int data, List <Node> dijkstra, Node stackedNode, Node prime, boolean x, TextArea textArea) {
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("simulation.txt", true))) {
			...
			if(transmittance > packageData) {
				...
				if (faulting != 1.0 && rand < 0.5) {
					...
					textArea.appendText("- Overload! -  Half ammount of data must be sent again (" + loss + ")."); bw.write("- Overload! -  Half ammount of data must be sent again (" + loss + ").\n\n");
	...
	/**
	 * This method is watching node of the graph.
	 * 
	 * @param text  Text from first TextField for removing edge.
	 * @param textArea  TextArea of GUI.
	 */
